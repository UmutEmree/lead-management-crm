                                ...
                                <tbody>
                                   @foreach($brands as $brand)
                                    <tr>
                                        ...
                                        <td>{{ $brand->name }}</td>                                    
                                        <td>{{ $brand->limit }}</td>                                    
                                        <td> <button class="btn btn-success waves-effect" type="button" onclick="location.href='{{ url('/admin/marka-duzenle/'.$brand->id) }}'">
                                                    <i class="material-icons">Düzenle</i>
                                                </button> /  <button class="btn btn-danger waves-effect" type="button" onclick="deleteTag({{ $brand->id }})">
                                                    <i class="material-icons">Sil</i>
                                                </button>
                                                <form id="delete-form-{{ $brand->id }}" action="{{ url('admin/delete-brand',$brand->id) }}" method="POST" style="display: none;">
                                                    {{ csrf_field() }}
